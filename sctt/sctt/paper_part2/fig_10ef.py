...
ctt = CompositeTensileTest(n_x=1000,
                           L=500.,
                           cb=cb)
# mean_arr = np.array([2.8, 2.6, 2.8, 3.3, 3.2])
# stdev_arr = np.array([0.15, 0.20, 0.15, 0.25, 0.10])

mean_arr = np.array([3.0 * 1.35])
...
for m, s in zip(mean_arr, stdev_arr):
    ...
    plt.figure()
    plt.plot(np.hstack((sig_c_i, sig_c_u)), cs, drawstyle='steps', lw=2)
