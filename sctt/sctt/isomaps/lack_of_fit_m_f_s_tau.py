...
def plot_m_f_s_tau(m_f, s_f, m_tau, s_tau, w_arr, sig_w):
    ...
    X, Y = np.meshgrid(m_f_arr, s_tau_arr)
    ...
    im = plt.imshow(delta, interpolation='bilinear', origin='lower',
                    cmap=cm.gray, extent=(0.75 * m_f, 1.25 * m_f, 0.75 * s_tau, 1.25 * s_tau), aspect=m_f / s_tau)
    levels = np.arange(0, 5, 0.2)
    CS = plt.contour(delta, levels,
                     origin='lower',
                     linewidths=2,
                     extent=(0.75 * m_f, 1.25 * m_f, 0.75 * s_tau, 1.25 * s_tau))
    ...
    plt.plot(m_f, s_tau, 'ro')
    plt.xlabel('m_f')
    plt.ylabel('s_tau')
    path = 'D:\\fig\\m_f_s_tau'
    plt.savefig(path)
