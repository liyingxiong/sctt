...
def plot_s_f_m_tau(m_f, s_f, m_tau, s_tau, w_arr, sig_w):
    ...
    X, Y = np.meshgrid(s_f_arr, m_tau_arr)
    ...
    for i in range(n):
        for j in range(n):
            print(j)
            delta[i, j] = lackoffit(
                m_f, X[i, j], Y[i, j], s_tau, w_arr, sig_w)
    ...
    im = plt.imshow(delta, interpolation='bilinear', origin='lower',
                    cmap=cm.gray, extent=(0.75 * s_f, 1.25 * s_f, 0.75 * m_tau, 1.25 * m_tau), aspect=s_f / m_tau)
    levels = np.arange(0, 5, 0.2)
    CS = plt.contour(delta, levels,
                     origin='lower',
                     linewidths=2,
                     extent=(0.75 * s_f, 1.25 * s_f, 0.75 * m_tau, 1.25 * m_tau))
    ...
    plt.plot(s_f, m_tau, 'ro')
    plt.xlabel('s_f')
    plt.ylabel('m_tau')
    path = 'D:\\fig\\s_f_m_tau'
    plt.savefig(path)
