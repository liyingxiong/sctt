...
def plot_f(m_f, s_f, m_tau, s_tau, w_arr, sig_w):
    ...
    n = 80
    m_f_arr = np.linspace(0.3 * m_f, 1.25 * m_f, n)
    s_f_arr = np.linspace(0.3 * s_f, 1.25 * s_f, n)

    X, Y = np.meshgrid(m_f_arr, s_f_arr)
    ...
    for i in range(n):
        for j in range(n):
            print(j)
            delta[i, j] = lackoffit(
                X[i, j], Y[i, j], m_tau, s_tau, w_arr, sig_w)
    ...
    im = plt.imshow(delta, interpolation='bilinear', origin='lower',
                    cmap=cm.gray, extent=(0.3 * m_f, 1.25 * m_f, 0.3 * s_f, 1.25 * s_f), aspect=m_f / s_f)
    levels = np.arange(0, 10, 0.2)
    CS = plt.contour(delta, levels,
                     origin='lower',
                     linewidths=1,
                     extent=(0.3 * m_f, 1.25 * m_f, 0.3 * s_f, 1.25 * s_f))
...
if __name__ == '__main__':
    ...
    plot_f(21.259, 0.0142,
           0.0965, 0.718, w_arr, sig_w)
